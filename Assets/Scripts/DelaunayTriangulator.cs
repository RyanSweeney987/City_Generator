...
/*
 * This is a quick Unity port, original copyright belongs to
...
namespace DelaunayTriangulator
{
    ...
    class Triangulator
    {
        ...
        private void Analyse(List<Vertex> suppliedPoints, Hull hull, List<Triad> triads, bool rejectDuplicatePoints, bool hullOnly)
        {
            ...
            Debug.Log((points.Count - nump).ToString() + " duplicate points rejected");
        ...
        public List<Triad> Triangulation(List<Vertex> points, bool rejectDuplicatePoints)
        {
            ...
            {
                tri.va = points[tri.a].ToVector2();
                tri.vb = points[tri.b].ToVector2();
                tri.vc = points[tri.c].ToVector2();
                tri.circumCenter = new Vector2(tri.circumcircleX, tri.circumcircleY);
            }
